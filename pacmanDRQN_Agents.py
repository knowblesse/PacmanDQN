...
class PacmanDRQN(game.Agent):
    def __init__(self):
        ...
        self.exp_num = datetime.datetime.now().strftime('%Y%m%d_%H:%M:%S')
        self.writer = SummaryWriter(f'./runs/{self.model_name}/{self.exp_num}')
        self.logger = logging.getLogger()
        self.file_handler = logging.FileHandler(f'./logs/{self.model_name}/{self.exp_num}.log')
        ...
        self.tau = 0.01
        self.gamma = 0.9
        self.train_size = 64 # minimum number of episodes required to train the agent's model
        ...
        #     self.drqn_pred = nn.DataParallel(self.drqn_pred)
        #     self.drqn_targ = nn.DataParallel(self.drqn_targ)
    ...
    def final(self, state):
        ...
        self.writer.add_scalar('Epsilon vs episodes', self.epsilon, self.num_done_epi)
        self.logger.info(f'Episode: {self.num_done_epi}, Score: {score}, Reward: {self.reward_epi}, IsWin: {state.isWin()}, MemoryLen: {len(self.memory)}, numTrain: {self.num_train}')
    ...
    def getAction(self, state):
        ...
        a = self.algo(torch.flatten(Q), self.epsilon, possible_action_index)
    ...
    def train(self):
        ...
        observations = torch.FloatTensor(observations.reshape(self.batch_size * epi_length, 1, self.visualRange, self.visualRange)).to(self.device)
        ...
        next_observations = torch.FloatTensor(next_observations.reshape(self.batch_size * epi_length, 1,self.visualRange, self.visualRange)).to(self.device)
        dones = torch.FloatTensor(dones.reshape(self.batch_size * epi_length,-1)).to(self.device)

        h_target, c_target = self.drqn_targ.init_hidden_state(batch_size=1, training=True)
        ...
        h, c = self.drqn_pred.init_hidden_state(batch_size=1, training=True)
        q_out, _, _ = self.drqn_pred.forward(observations, h.to(self.device), c.to(self.device))
        q_out = q_out.view(q_out.size(0), self.n_action)
        Q = q_out.gather(1, actions)
...
class DRQN(nn.Module):
    ...
    def forward(self, x, h, c):
        x = F.relu(self.conv1(x)) # (N, 1, 7, 7)  -> (N, 32, 5, 5). RGB channel should be in second.
        x = F.relu(self.conv2(x)) # (N, 32, 5, 5) -> (N, 64, 3, 3).
        x = F.relu(self.conv3(x)) # (N, 64, 3, 3) -> (N, 64, 1, 1).
        x = x.view(x.size(0), -1) # (N, 64) remaining number of data (first dimension)
        x = x.unsqueeze(1) # (N, 64) -> (N, 1, 64)
        x, (new_h, new_c) = self.lstm(x, (h, c)) # (N, 1, 64)
        x = self.fc(x) # (N, 1, 5)
